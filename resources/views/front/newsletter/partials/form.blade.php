<form
    ...
    class="flex flex-col {{ ($stacked ?? false) ? '' : 'md:flex-row' }} items-stretch {{ $class ?? '' }}"
>
    <input type="text" name="username" style="display:none !important" tabindex="-1" autocomplete="off">

    <input class="mb-2 {{ ($stacked ?? false) ? '' : 'md:mb-0' }}" type="email" autocomplete="off" id="email" name="email" placeholder="developer@awesome.dev" aria-label="E-mail" required>
