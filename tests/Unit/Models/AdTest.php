...
it('can get an ad for the current date', function () {
    ...
    TestTime::freeze('Y-m-d H:i:s', '2017-12-31 00:00:00');
    expect(Ad::getForPage())->toBeNull();

    TestTime::freeze('Y-m-d H:i:s', '2018-01-01 01:00:00');
    expect(Ad::getForPage()->id)->toEqual($januaryAd->id);

    TestTime::freeze('Y-m-d H:i:s', '2018-01-15 00:00:00');
    expect(Ad::getForPage()->id)->toEqual($januaryAd->id);

    TestTime::freeze('Y-m-d H:i:s', '2018-01-31 00:00:00');
    expect(Ad::getForPage()->id)->toEqual($januaryAd->id);

    TestTime::freeze('Y-m-d H:i:s', '2018-02-01 00:00:00');
    expect(Ad::getForPage()->id)->toEqual($februaryAd->id);

    TestTime::freeze('Y-m-d H:i:s', '2018-02-15 00:00:00');
    expect(Ad::getForPage()->id)->toEqual($februaryAd->id);

    TestTime::freeze('Y-m-d H:i:s', '2018-02-28 00:00:00');
    expect(Ad::getForPage()->id)->toEqual($februaryAd->id);

    TestTime::freeze('Y-m-d H:i:s', '2018-03-01 00:00:00');
    expect(Ad::getForPage())->toBeNull();
});

test('an url specific ad will be displayed on that url', function () {
    TestTime::freeze('Y-m-d H:i:s', '2018-01-01 00:00:00');
